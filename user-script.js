...
ws.addEventListener("message",(e) => {
    ...
    document.getElementById("tab2").addEventListener("click", (event) => {
        ...
        // console.log(event.explicitOriginalTarget.innerText); // getting text
        // console.log(event.explicitOriginalTarget);
        rowInd = event.explicitOriginalTarget.parentNode.rowIndex; // start from 1
        colInd = event.explicitOriginalTarget.cellIndex; // start from 0

        // colInd == 0 means "Across" column, and colInd == 1 means "Down" column
        ...
        if (colInd === 0) {
            ...
            console.log(acrossData[rowInd - 1]);
        } else if (colInd === 1) {
            clue.innerHTML = "<b>clue : </b>" + downData[rowInd - 1].clue;
            // popup.classList.add("show");
            popup.style.visibility = "visible";
            console.log(downData[rowInd - 1]);
        }
